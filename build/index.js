...
class MyNotes {
  ...
  // edit method
  editNote(e) {
    var this_note = jquery__WEBPACK_IMPORTED_MODULE_0___default()(e.target).parents('li');
...
class Search {
  ...
  getResults() {
    const route_url = universityData.root_url + "/wp-json/university/v1/search?term=" + this.searchField.val();
    // console.log(route_url);

    jquery__WEBPACK_IMPORTED_MODULE_0___default().getJSON(route_url, results => {
      this.resultsDiv.html(`
                <div class="row">
                    <div class="one-third">
                        <h2 class="search-overlay__section-title">General Information</h2>
                            ${results.generalInfo.length ? '<ul class="link-list min-list">' : "<p>No general information matches that search.</p>"}
                                ${results.generalInfo.map(item => `<li><a href="${item.link}">${item.title}</a> ${item.post_type == "post" ? `by ${item.author_name}` : ""}</li>`).join("")}
                            ${results.generalInfo.length ? "</ul>" : ""}
                    </div>
                    <div class="one-third">
                        <h2 class="search-overlay__section-title">Programs</h2>
                            ${results.programs.length ? '<ul class="link-list min-list">' : "<p>No program matches that search.</p>"}
                    ...
                    <div class="one-third">
                        ...
                        ${results.events.length ? "" : `<p>No professors matches that search. <a href="${universityData.root_url}/events">View All Events </a></p>`}
    ...
    //         $.getJSON(universityData.root_url + "/wp-json/wp/v2/pages?search=" + this.searchField.val())).then(
    //             (posts, pages) => {
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkpallwelluniversity"] = self["webpackChunkpallwelluniversity"] || [];
